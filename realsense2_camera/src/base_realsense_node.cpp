...
#define FRAME_PRIME_HEIGHT 240
#define FRAME_PRIME_WIDTH 424
#define STEP 12
#define DEFAULT_THLD 0.95
...
static void output_unit_vector(unsigned int height, unsigned int width, float unitVectors[FRAME_MAX_HEIGHT][FRAME_MAX_WIDTH * 3])
{
	...
	std::cerr << "\nUnit Test for Realsense Unit Vector:\nOutput the saved data: frame width is " \
		<< width << " and frame height is " << height << " \n\n" << std::endl;

	std::cerr << "*** Beginning of unit vectors ***\n" << std::endl;

	fp = fopen("./realsense_unit_vectors.obj","w");
	if (!fp) {
		std::cerr << "\nFailed to open a file for realsense_unit_vectors.obj!\n\n" << std::endl;
		return;
	}

	std::cerr << "\nOutput unit_vectors to ./realsense_unit_vectors.obj!\n\n" << std::endl;
...
void BaseRealSenseNode::publishUnitVectorsTopic(const ros::Time& t, const std::map<stream_index_pair, bool>& is_frame_arrived)
{
	...
	static unsigned int height = 240, width = 424, sequence;
	...
	if (unit_vector_mature || !publish_high_quality) {
		...
		if ((height == FRAME_PRIME_HEIGHT) && (width == FRAME_PRIME_WIDTH)) {
			image = new cv::Mat(height, width, CV_32FC3, current_unit_vectors, width * STEP);
		} else {
			for (unsigned int i = 0; i < height; i++)
				for (unsigned int j = 0; j < width * 3; j++)
					any_size_unit_vectors[i][j] = unit_vectors[i][j];

			image = new cv::Mat(height, width, CV_32FC3, any_size_unit_vectors, width * STEP);
		}
